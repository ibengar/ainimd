...
`.trim()
    ...
    */
    ...
    conn.fakeReply = async (jid, text = '', fakeJid = conn.user.jid, fakeText = '', fakeGroupJid, options) => {
      return await conn.reply(jid, text, { key: { fromMe: fakeJid == this.user.jid, participant: fakeJid, ...(fakeGroupJid ? { remoteJid: fakeGroupJid } : {}) }, message: { conversation: fakeText }, ...options })
    }
    /**
...
/*exports.smsg = (conn, m, hasParent) => {
     ...
     */
     m.reply = async (text, chatId, options) => {
        ...
        conn.reply(chatId ? chatId : m.chat, text, m, { contextInfo: { mentionedJid: conn.parseMention(text), externalAdReply: { title: 'Facebook', body: wm, sourceUrl: 'https://facebook.com/ribeng2', thumbnail: data }}, options })
    }
    m.name = m.pushName || conn.getName(m.sender)
